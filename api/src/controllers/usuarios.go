...
func SeguirUsuario(w http.ResponseWriter, r *http.Request) {
	...
	respositorio := repositorios.NovoRepositorioUsuarios(db)
...
func AtualizarSenha(w http.ResponseWriter, r *http.Request) {
	...
	var senhaNovaEAtual models.Senha
	if erro = json.Unmarshal(corpoDaRequisicao, &senhaNovaEAtual); erro != nil {
		respostas.Erro(w, http.StatusBadRequest, erro)
		return
	}

	repositorio := repositorios.NovoRepositorioUsuarios(db)
	senhaUsuario, erro := repositorio.BuscarSenha(usuarioId)
	if erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}

	if erro = seguranca.VerificarSenha(senhaUsuario, senhaNovaEAtual.Atual); erro != nil {
		respostas.Erro(w, http.StatusUnauthorized, errors.New("Senha atual inválida"))
		return
	}

	senhaNovaComHash, erro := seguranca.Hash(senhaNovaEAtual.Nova)
	if erro != nil {
		respostas.Erro(w, http.StatusBadRequest, erro)
		return
	}

	if erro = repositorio.AtualizarSenha(string(senhaNovaComHash), usuarioId); erro != nil {
		respostas.Erro(w, http.StatusBadRequest, erro)
		return
	}
